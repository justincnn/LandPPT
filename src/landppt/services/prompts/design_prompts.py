...
*   **文字完整显示**：所有文字（包括卡片内、列表项、标题等）必须完整可见，不能被截断、遮挡或超出容器边界。
...
**核心原则：文字必须100%可见、完整、清晰。任何视觉效果都必须服从于文字可读性。**

**动态自适应策略（根据实际内容智能调整）**：

1. **容器尺寸必须根据内容自动扩展**：
   - ✅ 所有包含文字的容器（卡片、列表项、内容框等）必须使用 `height: auto`
   - ✅ 如需限制尺寸，仅使用 `min-height`，绝不使用固定 `height`
   - ✅ 容器必须能够容纳所有文字内容而不截断
   - ❌ 严禁：固定高度的容器 + 超出内容（会导致文字被截断）

2. **文字必须自动换行而非溢出**：
   - ✅ 所有文字容器必须启用：`word-wrap: break-word; overflow-wrap: break-word;`
   - ✅ 中文长词必须能够在任意字符处断行
   - ✅ 确保文字在容器宽度内自动换行
   - ❌ 严禁：`white-space: nowrap` 或 `overflow: hidden` 导致文字不可见

3. **间距和内边距动态计算原则**：
   - 根据文字长度和容器大小，动态计算合适的 padding
   - 文字较多时：减小 padding，优先保证内容显示
   - 文字较少时：增大 padding，保持视觉美观
   - 确保内边距不会挤压文字空间导致溢出

4. **字体大小动态调整策略**：
   - **首要原则**：根据内容量自动调整字体大小，确保所有内容可见
   - **内容较多**：适当缩小字体（但保持可读性），使用 clamp() 设置最小可读尺寸
   - **内容适中**：使用标准字体大小
   - **内容较少**：可适当增大字体增强视觉冲击力
   - 使用 `font-size: clamp(最小值, 动态值, 最大值)` 确保在各种情况下都可读
   - **最小字体限制**：正文不小于 12px，标题不小于 14px（确保可读性）

5. **层级管理防止遮挡**：
   - 文字内容层的 z-index 必须高于所有装饰元素
   - 背景装饰、图案、边框等必须在文字下层
   - 使用 `position: relative` 确保文字层级可控
   - 确保没有任何元素（图片、装饰、边框）遮挡文字

6. **行高动态优化**：
   - 根据文字大小自动计算合适的行高
   - 文字较小时：使用较大的行高比（1.6-1.8）提高可读性
   - 文字较大时：使用适中行高比（1.3-1.5）
   - 确保多行文字不会重叠或贴得过近
...
7. **智能布局调整**：
   ...
   - **当卡片内容过多时**：
     ...
     * 或缩小卡片内字体
     * 或减少卡片内边距
   - **优先级**：内容完整显示 > 视觉美观 > 设计规范

8. **禁止的CSS模式**（这些会导致文字显示问题）：
   - ❌ `overflow: hidden` + 固定高度 + 超出内容
   - ❌ `text-overflow: ellipsis` 作为默认处理方式
   - ❌ `white-space: nowrap` 不配合容器宽度限制
   - ❌ 固定的小尺寸容器 + 大量文字
   - ❌ 装饰元素的 z-index 高于文字内容

**实施要点**：
- 🎯 在设计时，先评估内容量，再决定布局和字体大小
- 🎯 容器尺寸跟随内容，而非强制内容适应容器
- 🎯 当空间不足时，优先调整布局（分栏、网格）而非截断内容
- 🎯 使用 CSS 的 clamp()、auto、min-height、flex-grow 等特性实现自适应
- 🎯 测试思维：假设每个文字字段都可能很长，设计必须能够应对

**核心承诺：宁可牺牲部分美观度，也要确保所有文字100%可见且完整。**
