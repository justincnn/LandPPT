...
2. **所有内容容器必须使用以下约束之一**：
   - 使用 `max-height` 而非固定 `height`，让内容自适应
   - 绝对禁止在任何元素上设置 `overflow: auto`、`overflow: scroll` 或 `overflow-y: auto`
   - 所有容器必须显式设置 `overflow: hidden` 或 `overflow: visible`

3. **内容区域高度控制示例**：
```css
.main-content {{
    max-height: 600px;  /* 根据页眉页脚高度调整 */
    overflow: hidden !important;  /* 绝不使用 auto 或 scroll */
    display: flex;
...
4. **内容自适应策略**：
   - 使用 `flex-shrink: 1` 让内容在空间不足时自动收缩
   - 使用 `font-size: clamp()` 实现响应式字体大小
   - 使用多列布局（`column-count`）或网格布局分散内容
   - 动态调整 `gap`、`padding`、`margin` 来适应内容量
